...
// RequestObserver observes the actual HTTP transport work for one export.
// Attempt is called once for every exporter retry-loop invocation immediately
// before the request is sent. PayloadBytes is called as the HTTP transport
// reads the request body, including bodies replayed through Request.GetBody.
//
...
// WithRequestObserver installs an observer without replacing the exporter's
// HTTP client, preserving its endpoint, proxy, timeout, TLS, and mTLS behavior.
func WithRequestObserver(observer RequestObserver) Option {
	return wrappedOption{oconf.WithRequestObserver(observer)}
}
