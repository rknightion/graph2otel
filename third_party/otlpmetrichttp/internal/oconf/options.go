...
//
// Modified by the graph2otel project in 2026 to carry RequestObserver through
// HTTP exporter configuration. See the repository root LICENSE for the
// combined work.
...
type (
	// RequestObserver observes HTTP retry-loop invocations and request-body
	// bytes. It is internal so the public exporter package can expose the
	// extension without creating an import cycle.
...
func WithRequestObserver(observer RequestObserver) GenericOption {
	return newGenericOption(func(cfg Config) Config {
		cfg.Metrics.Observer = observer
		return cfg
	})
}
