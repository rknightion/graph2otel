...
Run from grafana/ (``just rules`` / ``just grafana-check`` do). Sibling of
``build_dashboard.py`` (#218) — same sys.path setup, same pure-stdlib
constraint (no PyYAML, no ``setup-python`` step in CI: see the justfile and
...
<!-- GENERATED by grafana/build_rules.py from its HUNTS list — do not edit by
     hand. Edit the hunt there, then run `just rules`. Every query below is built
...
# DETECTIONS list there, then run `just rules`.
#
# Grafana App Platform AlertRule (rules.alerting.grafana.app/v0alpha1). Deploy
# with `just rules-push` (see docs/deploying-observability.md), NOT with a bare
...
# dashboards/graph2otel.json. Change the dashboard, re-run `just rules`. The
...
def main() -> int:
    ...
    if stale:
        print(f"\nSTALE GENERATED FILES ({len(stale)}) — run `just rules`:", file=sys.stderr)
        for s in stale:
