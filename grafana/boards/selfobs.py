...
from builder import DASHBOARD_NAME, RATE, TENANT_SEL
...
def _drilldown(summary, target):
    summary["fieldConfig"]["defaults"]["links"] = [{
        ...
        "url": (
            f"/d/{DASHBOARD_NAME}?viewPanel={target['id']}"
...
def extra(b):
    b.variable(
        "collector",
        f'label_values({COLLECTOR_AVAILABILITY}{{tenant_id=~"$tenant"}}, collector)',
        label="Collector", multi=True, include_all=True,
    )
