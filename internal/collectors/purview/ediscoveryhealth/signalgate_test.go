package ediscoveryhealth

import (
	"testing"

	"github.com/rknightion/graph2otel/internal/signalcapture"
)

// TestMain enforces #112 over everything this package's tests emit: no
// metric label may carry per-entity data (case id/hold id/source id/display
// names must stay in the log twins, never on the purview.ediscovery.*
// gauges). See internal/signalcapture.
func TestMain(m *testing.M) { signalcapture.Main(m) }
