...
// sampleRows use the four columns live-confirmed from the
// AutopilotV2DeploymentStatusDetailedScriptInfo export header (probed as
// graph2otel-poller 2026-07-20 on m7kni). The report returned ZERO rows on m7kni
// (no device-prep scripts configured), so the column NAMES are live-confirmed but
// the field VALUES here are illustrative — used only to pin the column→attr wiring,
// never to assert a value exists on the wire (the #142 discipline).
func sampleRows() []exportjob.Row {
	return []exportjob.Row{
		{
			"DeviceId":    "eacc407b-5c7a-40f5-a98a-d803198bb768",
			"PolicyId":    "b1a2c3d4-0000-0000-0000-000000000001",
			"DisplayName": "Set-TimeZone", "PolicyInstallStatus": "2",
		},
		{
			"DeviceId":    "69fb6f6d-8be7-4552-b1d0-98afc0d629f7",
			"PolicyId":    "b1a2c3d4-0000-0000-0000-000000000002",
			"DisplayName": "Install-Certificates", "PolicyInstallStatus": "5",
		},
	}
}

// TestCollectCountsByInstallStatus pins the bounded gauge: one point per raw
// PolicyInstallStatus code, value = script-execution-row count.
func TestCollectCountsByInstallStatus(t *testing.T) {
	...
	want := map[string]float64{"2": 1, "5": 1}
	points := rec.MetricPoints(metricName)
	if len(points) != len(want) {
		t.Fatalf("got %d points, want %d: %+v", len(points), len(want), points)
	}
	for _, p := range points {
		if p.Value != want[p.Attrs[semconv.AttrPolicyInstallStatus]] {
			t.Errorf("status %q = %v", p.Attrs[semconv.AttrPolicyInstallStatus], p.Value)
		}
		if p.Unit != "{script}" {
			t.Errorf("Unit = %q, want {script}", p.Unit)
		}
	}
}

// TestMetricCarriesOnlyBoundedDimensions: many distinct (device, script) rows on one
...
func TestMetricCarriesOnlyBoundedDimensions(t *testing.T) {
	...
	for i := range 40 {
		rows = append(rows, exportjob.Row{
			"DeviceId": fmt.Sprintf("dev-%d", i), "PolicyId": fmt.Sprintf("pol-%d", i),
			"DisplayName": fmt.Sprintf("Script %d", i), "PolicyInstallStatus": "2",
		})
	}
	...
	for k := range points[0].Attrs {
		if k != semconv.AttrPolicyInstallStatus {
			t.Errorf("metric carries unbounded attribute %q; per-script detail belongs on the %s twin (#83, #112)", k, eventName)
		}
	}
}

// TestCollectEmitsTwinPerRow pins the twin: one record per (device, script) row with
// the full per-script detail and INFO severity.
func TestCollectEmitsTwinPerRow(t *testing.T) {
	...
	byScript := map[string]telemetrytest.LogRecord{}
	for _, l := range logs {
		...
		byScript[l.Attrs[semconv.AttrDisplayName]] = l
	}
	got := byScript["Set-TimeZone"]
	want := map[string]string{
		semconv.AttrDeviceId:            "eacc407b-5c7a-40f5-a98a-d803198bb768",
		semconv.AttrPolicyId:            "b1a2c3d4-0000-0000-0000-000000000001",
...
// (no device-prep scripts) emits no gauge points, no twins, no error.
