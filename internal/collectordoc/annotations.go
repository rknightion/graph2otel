...
var annotations = map[string]Annotation{
	...
	"entra.access_reviews": {
		Collects: "Entra ID Governance access-review DEFINITION inventory (#260) — a review that is created and then never completed is a governance control that looks present and does nothing. Bounded gauge counts definitions by `status`; one log twin per definition carries display name, descriptions, timestamps, the polymorphic scope read by its `@odata.type` (principal + resource queries, so a \"review the Global Administrators\" review names the roleDefinition it targets), reviewer identities and counts, cadence, and the governance settings, and Warns when neither notifications nor reminders are enabled. Review INSTANCES (recurrences) are deliberately out of scope — the expanded inline `instances` array is EMPTY even for an `InProgress` review on both v1.0 and beta (live 2026-07-24), so real per-instance progress needs an N×M fan-out — which means this collector reports that a review exists and how it is configured, but CANNOT tell you whether it is being completed, and makes no staleness claim. `status` is deliberately left unwatched by `wirecheck`: only one value has ever been observed, and #234 forbids declaring an enum from documentation",
		Source:   "`/identityGovernance/accessReviews/definitions` (v1.0 — the endpoint also answers on beta, so no `Experimental` gate; beta additionally returns `backupReviewers`/`customData`/`customDataProvider`, which are therefore not mapped)",
