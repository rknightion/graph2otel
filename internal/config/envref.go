...
// Default()'s key set by TestExampleConfigCoversEveryKey, so the example is a
// faithful, single source for the reference (it documents recommended values,
// so the values may differ from the bare defaults — TestConfigExampleLoadsAndValidates
// guards that it still loads and validates). The generated table lives between
// these markers; prose outside them is hand-written.
const (
...
type envRow struct {
	...
	EnvVar   string // G2O_* variable name ("" when file-only)
...
func envReferenceRows(exampleYAML []byte) ([]envRow, error) {
	...
	walk = func(prefix, inherited string, m *yaml.Node) {
		for i := 0; i+1 < len(m.Content); i += 2 {
			...
			// mapping ({}) is a leaf (a file-only map like profiling.pyroscope.tags).
			if v.Kind == yaml.MappingNode && len(v.Content) > 0 {
				// Children that carry no comment of their own inherit this
				// section's comment.
...
func leafRow(key string, k, v *yaml.Node, inherited string) envRow {
	...
	case yaml.SequenceNode:
		...
		if listEnvKeys[key] {
			...
			r.FileOnly = true // list of structs (tenants)
		}
	default: // scalar
