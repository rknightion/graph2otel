...
// Secret is a string-typed config value that holds a credential (token,
// password). It redacts itself in all formatting and logging output so a
// stray slog/fmt dump of the config can never leak the value; call Reveal to
// obtain the real string at the point of legitimate use.
type Secret string

// Reveal returns the underlying secret value for legitimate use (e.g. building
// an Authorization header). This is the only way to read the value, so
// credential reads are greppable.
func (s Secret) Reveal() string { return string(s) }

// redact reports the safe rendering: empty stays empty (nothing to hide), a
// set value becomes "REDACTED".
