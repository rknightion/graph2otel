...
func TestSignalCatalogCapturesClientTelemetry(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			body:        `{"error":{"code":"InternalServerError","message":"Service unavailable."}}`,
			wantMetrics: []string{metricHTTPClientDuration, metricHTTPClient5xx},
		},
		{
			name:        "429 throttle",
			status:      http.StatusTooManyRequests,
			body:        `{"error":{"code":"TooManyRequests","message":"Shared query budget exhausted."}}`,
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			_, queryErr := client.Query(context.Background(), "signal_capture", "DeviceTvmSoftwareVulnerabilities | take 1")
			if tc.status == http.StatusOK && queryErr != nil {
				t.Fatalf("Query: %v", queryErr)
			}
			if tc.status != http.StatusOK && queryErr == nil {
				t.Fatalf("Query status %d = nil error, want typed service error", tc.status)
			}
