...
- **Telemetry emitter facade** (`internal/telemetry`) — the only thing touching OTLP.
  ...
  **Undedupeable is degraded; misdated is wrong — only wrong justifies a drop.**
  The mirror case is the opposite call: a record whose event time is KNOWN but older
  than the backend accepts is **relocated, not dropped** — `WithBackdateClamp` moves it
  to ingestion time and keeps the truth on `event_time` + `event_time_clamped` (#226).
  Grafana Cloud discards backdated logs past **~4h** with no error of any kind
  (`live-measured 2026-07-22, #226`), so "the push succeeded" is not evidence a record
  landed. Treat ~4h as the ceiling; see `docs/signals.md`.
