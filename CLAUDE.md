...
- **Namespaces:** domain metrics use `entra.*` / `intune.*` / `m365.*` / `purview.*`;
  ...
  namespace is a bug.
- **`tenant_id` is on self-obs signals ONLY** (#143, live-verified): nothing injects it
  into domain metrics **or** domain logs — their labels/attributes are exactly what the
  collector passed. Do not filter domain-signal dashboards by `tenant_id` (returns no
  data, always); #143 owns the design decision.
- **No provenance attribute exists yet** (#141): a blob-ingested and a Graph-polled
  record are byte-identical by design (shared mappers). When one is added it must NOT be
  named `source` — that key is already overloaded three ways.
