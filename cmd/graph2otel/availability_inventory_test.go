...
func TestAvailabilityCandidatesCoverCompleteSevenPathCensus(t *testing.T) {
	...
	if len(first) != 157 {
		t.Fatalf("availabilityCandidates() count = %d, want 157 registration-path candidates", len(first))
	}
	...
	if len(byName) != 154 {
		t.Fatalf("availability logical-name count = %d, want 154", len(byName))
	}
	for _, candidate := range first {
		for _, peer := range candidate.ConflictsWith {
...
func TestResolveAvailabilityInventoryIsCompleteBoundedAndDeterministic(t *testing.T) {
	...
	if len(first) != 154 {
		t.Fatalf("inventory count = %d, want 154 logical collectors", len(first))
	}
