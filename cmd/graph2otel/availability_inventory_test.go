...
func TestAvailabilityCandidatesCoverCompleteSevenPathCensus(t *testing.T) {
	...
	if len(first) != 173 {
		t.Fatalf("availabilityCandidates() count = %d, want 173 registration-path candidates", len(first))
	}
	...
	if len(byName) != 170 {
		t.Fatalf("availability logical-name count = %d, want 170", len(byName))
	}
	for _, candidate := range first {
		for _, peer := range candidate.ConflictsWith {
...
func TestResolveAvailabilityInventoryIsCompleteBoundedAndDeterministic(t *testing.T) {
	...
	if len(first) != 170 {
		t.Fatalf("inventory count = %d, want 170 logical collectors", len(first))
	}
