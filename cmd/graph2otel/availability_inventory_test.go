...
func TestAvailabilityCandidatesCoverCompleteSevenPathCensus(t *testing.T) {
	...
	if len(first) != 169 {
		t.Fatalf("availabilityCandidates() count = %d, want 169 registration-path candidates", len(first))
	}
	...
	if len(byName) != 166 {
		t.Fatalf("availability logical-name count = %d, want 166", len(byName))
	}
	for _, candidate := range first {
		for _, peer := range candidate.ConflictsWith {
...
func TestResolveAvailabilityInventoryIsCompleteBoundedAndDeterministic(t *testing.T) {
	...
	if len(first) != 166 {
		t.Fatalf("inventory count = %d, want 166 logical collectors", len(first))
	}
