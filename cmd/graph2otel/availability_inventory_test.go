...
func TestAvailabilityCandidatesCoverCompleteSevenPathCensus(t *testing.T) {
	...
	if len(first) != 167 {
		t.Fatalf("availabilityCandidates() count = %d, want 167 registration-path candidates", len(first))
	}
	...
	if len(byName) != 164 {
		t.Fatalf("availability logical-name count = %d, want 164", len(byName))
	}
	for _, candidate := range first {
		for _, peer := range candidate.ConflictsWith {
...
func TestResolveAvailabilityInventoryIsCompleteBoundedAndDeterministic(t *testing.T) {
	...
	if len(first) != 164 {
		t.Fatalf("inventory count = %d, want 164 logical collectors", len(first))
	}
