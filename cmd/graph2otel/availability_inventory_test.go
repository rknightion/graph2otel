...
func TestAvailabilityCandidatesCoverCompleteSevenPathCensus(t *testing.T) {
	...
	if len(first) != 165 {
		t.Fatalf("availabilityCandidates() count = %d, want 165 registration-path candidates", len(first))
	}
	...
	if len(byName) != 162 {
		t.Fatalf("availability logical-name count = %d, want 162", len(byName))
	}
	for _, candidate := range first {
		for _, peer := range candidate.ConflictsWith {
...
func TestResolveAvailabilityInventoryIsCompleteBoundedAndDeterministic(t *testing.T) {
	...
	if len(first) != 162 {
		t.Fatalf("inventory count = %d, want 162 logical collectors", len(first))
	}
