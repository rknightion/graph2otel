...
| Intune `OperationalLogs` | — | **No Graph endpoint, unchanged** (#94) — Graph exposes only the notification templates, never the compliance-alert *fired-event* stream. **Buildable via the same blob escape hatch as `entra.graph_activity`:** `insights-logs-operationallogs` now exists on the verification tenant with a live fired-alert sample — live-measured (2026-07-17, #171). The sample carries an `OperationalLogCategory` field (`DeviceCompliance` observed), which suggests more than one alert type lands in this container; sample wider before mapping. Not yet built. |
