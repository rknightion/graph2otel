...
| `entra.licensing` | SKU consumption + prepaid/enabled units | `/subscribedSkus` | `LicenseAssignment.Read.All` | — | 15m | `entra.license.consumed{sku}`, `entra.license.enabled{sku}` |
| `entra.mfa_registration` | MFA/SSPR/passwordless registration + capability status, per-method counts, admin MFA-capable split | `/reports/authenticationMethods/userRegistrationDetails` | `AuditLog.Read.All` | `needs-license/entra_p1` | 1h | `entra.mfa.registration.admin_mfa_capable.total{is_admin}`, `entra.mfa.registration.methods.total{method}`, `entra.mfa.registration.users.total{status}`, plus a log twin per `entra.user_registration` |
