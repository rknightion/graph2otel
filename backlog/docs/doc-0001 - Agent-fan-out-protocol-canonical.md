...
updated_date: '2026-08-14 16:25'
---
> **Imported verbatim from `~/repos/agent-fanout-generic.md` on 2026-08-14.** This in-repo copy is
> authoritative for graph2otel: it is tracked, so an agent with only this checkout — in CI, on
> another machine — has the whole model, while the source is a loose unversioned file on one
> machine. Everything below this line is the source, unmodified. Do not summarise, compress or
> "adapt" it here; when the source changes, re-import the whole file rather than patching this copy,
> and the source edit is not finished until every consuming repo has been re-imported in the same
> change.
